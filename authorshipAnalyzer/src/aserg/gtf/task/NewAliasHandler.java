...
public class NewAliasHandler{
	private static final Logger LOGGER = Logger.getLogger(NewAliasHandler.class);
	...
	private void treatFileAlias() {
		for (LineInfo info : fileAliases) {
			...
			if (devNameMap.containsKey(dev1.toUpperCase()) && devNameMap.containsKey(dev2.toUpperCase())) {
				String usernameDev1 = devNameMap.get(dev1.toUpperCase()).get(0)
						.getUserName();
				String usernameDev2 = devNameMap.get(dev2.toUpperCase()).get(0)
						.getUserName();
				String newUsername = usernameDev1.contains(usernameDev2) ? usernameDev1
						: (usernameDev2.contains(usernameDev1) ? usernameDev2
								: usernameDev1 + "$$" + usernameDev2);
				...
				LOGGER.warn("No authors for alias: " + dev1 + " - "+ dev2);
			}
